...
analyze(Files, IgnoreSpecsFromState, Rules, ParsingStyle, Context) ->
    ...
    WhollyIgnoredFiles =
        lists:usort(
            [File || {File, all, _Options} <- IgnoreSpecsFromState] ++
...
remove_ignored_results(
    ...
    NewAcc =
        case
            lists:search(
                fun(IgnoreSpec) -> hank_rule:is_ignored(Rule, Pattern, IgnoreSpec) end,
                IgnoreSpecs
            )
        of
            {value, IgnoreSpec} ->
...
ignore_specs(File, Rule, IgnoreRules) ->
    Fun = fun
        ({File0, Rule0, Specs}, IgnoreSpecs) when
            File =:= File0 andalso Rule =:= Rule0
        ->
